...
async def test_metadata_for_node():
    ...
    patch_query(
        ...
        [
            ...
            {
                "p": _uri("http://dbpedia.org/ontology/iupacName"),
                "o": _literal("1-desamino-8-D-arginine vasopressin"),
            },
        ],
    )
...
async def test_end_to_end_bfs_via_cached_db():
    ...
    async def fake_query(sparql: str) -> list[dict]:
        ...
        if "FILTER(isLiteral(?o))" in sparql:
            ...
            for eid, label in [
                (ID_DESMO, "Desmopressin"),
                (ID_CUSHINGS, "Cushing's disease"),
                (ID_AVPR2, "AVPR2"),
                (ID_HYPER, "Hypertension"),
            ]:
